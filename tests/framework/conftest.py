...
def make_structured_evidence(
    ...
    body = torch.stack(
        [
            torch.randint(num_preds, (B, P, D, M), generator=g),
            torch.randint(num_ents, (B, P, D, M), generator=g),
            torch.randint(num_ents, (B, P, D, M), generator=g),
        ],
        dim=-1,
    )
    ...
    rule_idx = torch.randint(2, (B, P, D), generator=g)
...
def make_legacy_evidence(
    ...
    body = torch.stack(
        [
            torch.randint(num_preds, (B, P, G_body), generator=g),
            torch.randint(num_ents, (B, P, G_body), generator=g),
            torch.randint(num_ents, (B, P, G_body), generator=g),
        ],
        dim=-1,
    )
    mask = torch.ones(B, P, dtype=torch.bool)
    count = torch.full((B,), P, dtype=torch.long)
    rule_idx = torch.zeros(B, P, dtype=torch.long)
    body_count = torch.full((B, P), G_body, dtype=torch.long)
    return FakeProofEvidence(
        body=body, mask=mask, count=count, rule_idx=rule_idx,
